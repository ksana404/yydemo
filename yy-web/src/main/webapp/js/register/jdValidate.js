...
function checkPin(option) {
    ...
    if (!namestate || nameold != pin) {
        if (nameold != pin) {
            ...
            $.ajax({
            	url : "http://sso.yy.com/user/check/"+escape(pin)+"/1?r=" + Math.random(),
            	dataType : "jsonp",
            	success : function(data) {
                  ...
                  if(data.status==200){
                	  ...
                	  validateSettings.error.run(option, "服务器繁忙,请稍后再试！");
                      namestate = false;
                  }
                }
            });
...
function sendMobileCode() {
    ...
    // 检测手机号码是否存在
    $.ajax({
    	url : "http://sso.yy.com/user/check/"+mobile+"/2?r=" + Math.random(),
    	dataType : "jsonp",
